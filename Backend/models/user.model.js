...
export const createUser = async (fullname, email, password, designation = '', department = '', phone = '') => {
  const query = `
    INSERT INTO users (fullname, email, password, designation, department, phone)
    VALUES ($1, $2, $3, $4, $5, $6)
    RETURNING *;
  `;
  const values = [fullname, email, password, designation, department, phone];
...
export const updateUser = async (id, updates) => {
  ...
  const allowed = ["fullname", "email", "password", "designation", "department", "phone"];
  const setClauses = [];
  const values = [];
  let idx = 1;
...
export const searchUsers = async (searchTerm) => {
  const query = `
        SELECT id, fullname, email, designation, profile_picture 
        FROM users 
